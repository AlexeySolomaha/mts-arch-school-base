...
| QR.003 | Система должна выполнять свои функции и не прерывать проводимые трансляции в пиковые моменты нагрузки (1000 пользователей одновременно)|
| QR.004 | Система должна поддерживать резервирование для предотвращения потери данных о мероприятиях и докладчиках |
| QR.005 | Система должна поддерживать поддерживать работу в наиболее популярных современных браузерах на базе Chrome/Chromium |
...
| QR.008 | Система должна иметь возможность планировать 2 конференции в месяц.
| QR.009 | Система должна иметь возможность осуществлять рецензирование 100 докладов от кандидатов для каждой конференции.
| QR.010 | Система должна иметь возможность хранить материалы 100 докладов для каждой прошедшей конференции.
| QR.011 | Срок поставки функционала системы - 9 месяцев с момента утвержения требований |

### Бизнес-метрики/Нефункциональные требования/Показатели качества
|Бизнес-метрика|Нефункциональные требования|Показатели качества|
|--------------|---------------------------|-------------------|
|Сайт доступен 95% времени| Допустимое среднее время восстановления (MTTR) - 2 часа (среднее время на устранение неполадок аппаратного обеспечения и перезагрузку ПО)|Среднее время наработки на отказ не должно превышать MTBF = MTTR * 95%/(100% - 5%)=40 часов при заданном MTTR = 2 часа| 
